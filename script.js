...
// ========================================
// GROUP FUNCTIONALITY - GitHub Gist System (Backup)
// ========================================

// Group variables (already declared at top)
// let currentGroup = null;
...
async function doCreateGroup() {
    ...
    try {
        ...
        const gistData = {
            description: `Zikirmatik Group: ${groupName}`,
            public: false,
            files: {
                'group-data.json': {
        ...
        if (!response.ok) {
            if (response.status === 401) {
                // GitHub requires authentication - use local storage fallback
                console.log('GitHub auth required, using local storage system');
                createLocalGroup(groupName, creatorName, groupCode);
                return;
            }
        ...
        showCustomAlert('❌ Grup oluşturulamadı!<br>İnternet bağlantınızı kontrol edin.', 'error', 4000);
        hideStatus();
    }
}
...
async function doJoinGroup() {
    ...
    const participantNameInput = document.getElementById('participantNameInput').value.trim();
    ...
    try {
        // Search for group by code - we'll search through recent gists
        const gistId = await findGroupByCode(groupCode);
        ...
        // Get group data
        const groupData = await getGroupData(gistId);

        // Check if participant name already exists
        if (groupData.participants[participantNameInput]) {
            showCustomAlert('⚠️ Bu isim zaten kullanılıyor!<br>Farklı bir isim deneyin.', 'warning', 3000);
            hideStatus();
            return;
        }

        // Add participant to group
        groupData.participants[participantNameInput] = {
            stats: getCurrentUserStats(),
            lastUpdate: new Date().toISOString()
        };
        ...
        // Update gist
        await updateGroupData(gistId, groupData);
        ...
        showCustomAlert(`✅ "${groupData.groupInfo.name}" grubuna katıldınız!`, 'success', 3000);

    } catch (error) {
        console.error('Error joining group:', error);
        showCustomAlert('❌ Gruba katılamadı!<br>Kod doğru mu? İnternet bağlantınızı kontrol edin.', 'error', 4000);
        hideStatus();
    }
}

// Find group by code (simplified approach)
async function findGroupByCode(code) {
    // For now, we'll use a simple mapping approach
    // In a real implementation, you might use a public gist as an index
    // For this demo, we'll store the mapping in localStorage
    const groupMappings = JSON.parse(localStorage.getItem('groupMappings') || '{}');
    return groupMappings[code] || null;
}

// Get group data from gist
async function getGroupData(gistId) {
    const response = await fetch(`${GIST_API_URL}/${gistId}`);
    if (!response.ok) {
        throw new Error('Grup verisi alınamadı');
    }
    const gist = await response.json();
    return JSON.parse(gist.files['group-data.json'].content);
}

// Update group data
async function updateGroupData(gistId, data) {
    const gistData = {
        files: {
            'group-data.json': {
    ...
    if (!response.ok) {
        throw new Error('Grup verisi güncellenemedi');
    }
}
...
function showGroupInterface(code) {
    ...
    if (isGroupCreator) {
        ...
        document.getElementById('statusMessage').textContent = `${currentGroup.name} grubunu yönetiyorsunuz`;
        document.getElementById('codeShare').style.display = 'block';
        document.getElementById('displayCode').textContent = code;

        // Store code mapping for others to find
        const groupMappings = JSON.parse(localStorage.getItem('groupMappings') || '{}');
        groupMappings[code] = currentGroup.gistId;
        localStorage.setItem('groupMappings', JSON.stringify(groupMappings));
    } else {
        document.getElementById('statusTitle').textContent = '👥 Grup Üyesi';
        document.getElementById('statusMessage').textContent = `${currentGroup.name} grubundasınız`;
...
function startPolling() {
    ...
    updateInterval = setInterval(updateLeaderboard, 10000); // Every 10 seconds
}
...
async function updateLeaderboard() {
    ...
    try {
        const groupData = await getGroupData(currentGroup.gistId);

        // Calculate leaderboard
        const participants = Object.entries(groupData.participants).map(([name, data]) => ({
            name: name,
            stats: data.stats,
...
function displayLeaderboard(participants) {
    ...
    participants.forEach((participant, index) => {
        const isMe = participant.name === participantName;
        const position = index + 1;
        let medal = '';
        ...
        html += `
            <div class="participant-row ${isMe ? 'my-row' : ''}">
                ...
                <div class="participant-info">
                    <div class="name">${participant.name} ${isMe ? '(Sen)' : ''}</div>
                    <div class="details">Bugün: ${participant.stats.today} • Hafta: ${participant.stats.week}</div>
                </div>
...
// Share group code
function shareCode() {
    const message = `🕌 Zikirmatik Grup Yarışmasına katıl!\n\nGrup: ${currentGroup.name}\nKod: ${currentGroup.code}\n\n📱 zikirmatik.github.io adresine git\n👥 "Grup" sekmesinde "Gruba Katıl"\n🔤 Kodu gir: ${currentGroup.code}`;
    ...
    if (navigator.share) {
        ...
        navigator.clipboard.writeText(message).then(() => {
            ...
            showCustomAlert('❌ Kopyalama hatası<br>Kod: ' + currentGroup.code, 'warning', 4000);
        });
    }
}
...
function refreshLeaderboard() {
    if (!currentGroup) return;
    showCustomAlert('🔄 Güncelleniyor...', 'success', 1000);
    updateLeaderboard();
}
...
// Send stats update when user increments counter
async function sendStatsUpdate() {
    if (!currentGroup || !participantName) return;

    try {
        const groupData = await getGroupData(currentGroup.gistId);

        if (groupData.participants[participantName]) {
            groupData.participants[participantName].stats = getCurrentUserStats();
            groupData.participants[participantName].lastUpdate = new Date().toISOString();
            groupData.lastUpdate = new Date().toISOString();

            await updateGroupData(currentGroup.gistId, groupData);
...
// Load group on startup if exists
function loadExistingGroup() {
    try {
        ...
        if (savedGroup && savedParticipant) {
            ...
            isGroupCreator = currentGroup.isCreator;

            showGroupInterface(currentGroup.code);
            startPolling();
        }
    } catch (error) {
        console.error('Error loading existing group:', error);
        localStorage.removeItem('currentGroup');
        localStorage.removeItem('participantName');
    }
}
